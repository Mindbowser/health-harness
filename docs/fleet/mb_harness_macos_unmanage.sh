#!/bin/sh
# Fleet script — macOS. UN-MANAGE the harness: remove the org-deployed Claude Code managed-settings.json
# so the plugin is no longer locked at managed scope. After this runs, the machine is USER-OWNED — each dev
# installs / updates / removes the harness themselves at user scope.
#
# Assumes managed-settings.json is harness-only (that's what mb_harness_macos.sh wrote). If your org later
